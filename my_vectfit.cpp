...
void calculate_column_poles_res_set(const cx_mat *f, const cx_vec *s, cx_mat *poles1, SER *wynik, int rr)
{
...
SER my_vf_column_splitting(const cx_mat *f, const cx_vec *s, cx_mat *poles)
{
    ...
    for ( int r = 0; r < column_num; r++ )
    { 
        calculate_column_poles_res_set(f, s, poles, &wynik, r);
    }
